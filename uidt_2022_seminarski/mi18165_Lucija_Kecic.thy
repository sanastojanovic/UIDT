...
(* HOLDER:
x1^(p+1)/y1^p + ... + xn^(p+1)/yn^p \<ge> (x1+...+xn)^(p+1)/(y1+...+yn)^p
...
lemma apply_holder_left_mid:
  ...
  assumes "a > 0" "b > 0" "c > 0"
  assumes "a^2 + b^2 + c^2 = 3"
  shows  "((a^2)^3) / (a^5 * (b + c - a)^2) + ((b^2)^3) / (b^5 * (a + c - b)^2)  + ((c^2)^3) / (c^5 * (a + b - c)^2) \<ge> (a^2 + b^2 + c^2)^3 / (sqrt(a^5) * (b + c - a) + sqrt(b^5) * (a + c - b) + sqrt(c^5) * (a + b - c))^2"
  using assms
  sorry
...
lemma apply_holder_left:
  ...
  assumes "a > 0" "b > 0" "c > 0"
  assumes "a^2 + b^2 + c^2 = 3"
  shows "a/(b+c-a)^2+b/(a+c-b)^2+c/(a+b-c)^2 \<ge> 27/(sqrt(a^5)*(b+c-a)+sqrt(b^5)*(a+c-b)+sqrt(c^5)*(a+b-c))^2"
proof -
  ...
  from fraca fracb fracc have eq1: "a/(b+c-a)^2+b/(a+c-b)^2+c/(a+b-c)^2 = ((a^2)^3)/(a^5*(b+c-a)^2)+((b^2)^3)/(b^5*(a+c-b)^2)+((c^2)^3)/(c^5*(a+b-c)^2)"
	by simp
  also have eq2: "((a^2)^3)/(a^5*(b+c-a)^2)+(b^2)^3/(b^5*(a+c-b)^2)+(c^2)^3/(c^5*(a+b-c)^2) \<ge> (a^2+b^2+c^2)^3/(sqrt(a^5)*(b+c-a)+sqrt(b^5)*(a+c-b)+sqrt(c^5)*(a+b-c))^2"
  ...
  have final: "(a^2+b^2+c^2)^3/(sqrt(a^5)*(b+c-a)+sqrt(b^5)*(c+a-b)+sqrt(c^5)*(a+b-c))^2 = 27/(sqrt(a^5)*(b+c-a)+sqrt(b^5)*(c+a-b)+sqrt(c^5)*(a+b-c))^2"
	using assms by auto
  from eq1 eq2 final show "a/(b+c-a)^2+b/(a+c-b)^2+c/(a+b-c)^2 \<ge> 27/(sqrt(a^5)*(b+c-a)+sqrt(b^5)*(a+c-b)+sqrt(c^5)*(a+b-c))^2"
...
lemma schur:
  ...
  shows "sqrt(a^3)*(a-b)*(a-c)+sqrt(b^3)*(b-a)*(b-c)+sqrt(c^3)*(c-a)*(c-b) \<ge> 0"
proof -
  have a1: "a-b \<ge> 0"
	using assms by auto
  have a2: "a-c \<ge> 0"
	using assms by auto
  have a3: "(a-b)*(a-c) \<ge> 0"
	using a1 a2 mult_nonneg_nonneg by blast
  then have "sqrt(a^3)*(a-b)*(a-c) \<ge> 0"
	using a1 a2 assms(1) by auto
  have b1: "b-c \<ge> 0"
	using assms by auto
  have b2:  "b-a \<le> 0"
	using assms by auto
  have b3: "(b-a)*(b-c) \<le> 0"
	using b1 b2 by (simp add: mult_nonneg_nonpos2)
  then have "sqrt(b^3)*(b-a)*(b-c) \<le> 0"
  ...
  then have "sqrt(b^3)*(-(a-b))*(b-c) \<le> 0"
	by simp
  then have "-sqrt(b^3)*(a-b)*(b-c) \<le> 0"
	by linarith
  ...
  then have ab: "(a-b)*(sqrt(a^3)*(a-c)-sqrt(b^3)*(b-c)) \<ge> 0"
	using a1 by auto
  have c1: "c-a \<le> 0"
	using assms by auto
  have c2: "c-b \<le> 0"
	using assms by auto
  have c3: "(c-a)*(c-b) \<ge> 0"
	using c1 c2 mult_nonpos_nonpos by blast
  then have c4: "sqrt(c^3)*(c-a)*(c-b) \<ge> 0"
	using c1 c2 by (smt (verit, del_insts) assms(3) mult_nonpos_nonpos real_sqrt_le_0_iff zero_less_mult_pos zero_less_power)
  show "sqrt(a^3)*(a-b)*(a-c)+sqrt(b^3)*(b-a)*(b-c)+sqrt(c^3)*(c-a)*(c-b) \<ge> 0"
...
lemma rewritten_schur:
  ...
  assumes "a^2+b^2+c^2 = 3"
  assumes "a \<ge> b" "b \<ge> c"
  shows "sqrt(a^5)*(b+c-a)+sqrt(b^5)*(c+a-b)+sqrt(c^5)*(a+b-c) \<le> a*b*c*(sqrt(a)+sqrt(b)+sqrt(c))"
proof -
  have schur1: "sqrt(a^3)*(a-b)*(a-c)+sqrt(b^3)*(b-a)*(b-c)+sqrt(c^3)*(c-a)*(c-b) \<ge> 0"
  ...
  then have "a*sqrt(a^5)-b*sqrt(a^5)-c*sqrt(a^5)+a*b*c*sqrt(a)+b*sqrt(b^5)-a*sqrt(b^5)-c*sqrt(b^5)+a*b*c*sqrt(b)+c*sqrt(c^5)-a*sqrt(c^5)-b*sqrt(c^5)+a*b*c*sqrt(c) \<ge> 0"
  ...
  then have "a*b*c*(sqrt(a)+sqrt(b)+sqrt(c))-sqrt(a^5)*(b+c-a)-sqrt(b^5)*(a+c-b)-sqrt(c^5)*(a+b-c) \<ge> 0"
	using abc1 abc2 schur1 by presburger
  then have "a*b*c*(sqrt(a)+sqrt(b)+sqrt(c)) \<ge> sqrt(a^5)*(b+c-a)+sqrt(b^5)*(a+c-b)+sqrt(c^5)*(a+b-c)"
	by linarith
  then show "sqrt(a^5)*(b+c-a)+sqrt(b^5)*(c+a-b)+sqrt(c^5)*(a+b-c) \<le> a*b*c*(sqrt(a)+sqrt(b)+sqrt (c))"
	by (smt (verit))
qed
...
lemma am_fpm:
  ...
  assumes "a>0" "b>0" "c>0"
  assumes "a^2+b^2+c^2 = 3"
  shows "((sqrt(a)+sqrt(b)+sqrt(c))/3)^4 \<le> (a^2+b^2+c^2)/3"
  sorry
...
proof -
  have "((sqrt(a)+sqrt(b)+sqrt(c))/3)^4 \<le> (a^2+b^2+c^2)/3"
	using assms am_fpm by auto
  then have "((sqrt(a)+sqrt(b)+sqrt(c))/3)^4 \<le> 1"
	using assms by auto
...
lemma left:
  ...
  shows "a/(b+c-a)^2+b/(a+c-b)^2+c/(a+b-c)^2 \<ge> 27/(a*b*c*(sqrt(a)+sqrt(b)+sqrt(c)))^2"
proof -
  have l1: "a/(b+c-a)^2+b/(a+c-b)^2+c/(a+b-c)^2 \<ge> 27/(sqrt(a^5)*(b+c-a)+sqrt(b^5)*(a+c-b)+sqrt(c^5)*(a+b-c))^2"
	using assms apply_holder_left by auto
  have "sqrt(a^5)*(b+c-a)+sqrt(b^5)*(c+a-b)+sqrt(c^5)*(a+b-c) \<le> a*b*c*(sqrt(a)+sqrt(b)+sqrt(c))"
	using assms rewritten_schur by auto
  then have "(sqrt(a^5)*(b+c-a)+sqrt(b^5)*(c+a-b)+sqrt(c^5)*(a+b-c))^2 \<le> (a*b*c*(sqrt(a)+sqrt(b)+sqrt(c)))^2"
	using assms by auto
  then have "1/(sqrt(a^5)*(b+c-a)+sqrt(b^5)*(c+a-b)+sqrt(c^5)*(a+b-c))^2 \<ge> 1/(a*b*c*(sqrt(a)+sqrt(b)+sqrt(c)))^2"
	using assms rewritten_schur by (smt (verit) frac_le zero_less_power)
  then have l2:  "27/(sqrt(a^5)*(b+c-a)+sqrt(b^5)*(c+a-b)+sqrt(c^5)*(a+b-c))^2 \<ge> 27/(a*b*c*(sqrt(a)+sqrt(b)+sqrt(c)))^2"
	by auto
  from l1 l2 show "a/(b+c-a)^2+b/(a+c-b)^2+c/(a+b-c)^2 \<ge> 27/(a*b*c*(sqrt(a)+sqrt(b)+sqrt(c)))^2"
...
lemma right:
  ...
  assumes "a*b*c > 0"
  assumes "a+b-c > 0" "b+c-a > 0" "c+a-b > 0"
  shows "27/(a*b*c*(sqrt(a)+sqrt(b)+sqrt(c)))^2 \<ge> 3/(a*b*c)^2"
proof -
  have "sqrt(a)+sqrt(b)+sqrt(c) \<le> 3"
	using assms sq_sum_lt_three by auto
  then have "a*b*c*(sqrt(a)+sqrt(b)+sqrt(c)) \<le> 3*a*b*c"
	using assms by simp
  then have "(a*b*c*(sqrt(a)+sqrt(b)+sqrt(c)))^2 \<le> (3*a*b*c)^2"
	using assms by auto
  then have "(a*b*c*(sqrt(a)+sqrt(b)+sqrt(c)))^2 \<le> 9*(a*b*c)^2"
	using assms by (simp add: power2_eq_square)
  then have "1/(a*b*c*(sqrt(a)+sqrt(b)+sqrt(c)))^2 \<ge> 1/(9*(a*b*c)^2)"
	using assms by (simp add: frac_le)
  then have "27/(a*b*c*(sqrt(a)+sqrt(b)+sqrt(c)))^2 \<ge> 27/(9*(a*b*c)^2)"
	by auto
  then show "27/(a*b*c*(sqrt(a)+sqrt(b)+sqrt(c)))^2 \<ge> 3/(a*b*c)^2"
	by auto
qed
...
lemma solution:
  ...
  shows "a/(b+c-a)^2 + b/(c+a-b)^2 + c/(a+b-c)^2 \<ge> 3/(a*b*c)^2"
proof -
  have l: "a/(b+c-a)^2+b/(a+c-b)^2+c/(a+b-c)^2 \<ge> 27/(a*b*c*(sqrt(a)+sqrt(b)+sqrt(c)))^2"
	using assms left by auto
  have r: "27/(a*b*c*(sqrt(a)+sqrt(b)+sqrt(c)))^2 \<ge> 3/(a*b*c)^2"
	using assms right by auto
  from l r show "a/(b+c-a)^2 + b/(c+a-b)^2 + c/(a+b-c)^2 \<ge> 3/(a*b*c)^2"
	using assms by (smt (verit))
qed
